...
class RagnarokBot:
    ...
    def __init__(self, window_offset,window_size):
        ...
        ## self.limestone_tooltip = cv2.imread('limestone_tooltip.jpg', cv2.IMREAD_UNCHANGED)
    ...
    def click_next_target(self):
        ...
        # loop:
        #   2. hover the nearest target
        #   3. confirm that it is a object vai the name 
        #   4. if not check the next target 
        # endloop
        # 5. if no target was found return false 
        # 6. click  on the found target return true
        targets = self.targets_ordered_by_distance(self.targets)

        targets_i = 0
        found_limestone = False
        while not found_limestone and targets_i < len(targets):
    ...
    def targets_ordered_by_distance(self, targets):
        ...
        print('targets_ordered_by_distance run')
        return targets

    def confirm_tooltip(self, target_position):
        # check the current screenshot for the limestone tooltip using match template
        result = cv2.matchTemplate(self.screenshot, self.limestone_tooltip, cv2.TM_CCOEFF_NORMED)
    ...
    def click_backtrack(self):
        ...
        pydirectinput.moveTo(x=int(screen_x), y=int(screen_y))
        # short pause to let the mouse movement complete
        sleep(0.500)
        print('click_backtrack run')
        pydirectinput.click()
        print('click_backtrack run pydirectinput.click()')
    ...
    def run(self):
        ...
        while not self.stopped:
            ...
            if self.state == BotState.INITIALIZING:
                # do no bot actions untill wait is complete
                if time() > self.timestamp + self.INITIALIZING_SECONDS:
                    # start searching when waiting is over
                    self.lock.acquire()
                    self.state = BotState.SEARCHING
                    self.lock.release()

            elif self.state == BotState.SEARCHING:
                # check the given position point targets, 
                # then click it.
